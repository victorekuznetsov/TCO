...
# реальные каталоги узлов ремонтов из файлов поставщиков: (наименование, стоим.замены тыс, ресурс м/ч, нормо-час)
# ресурс < 8000 → текущие ремонты; ресурс ≥ 8000 → капитальные (ППР)
_Z=[["Быстроизнашиваемые элементы",27.28,3000,24],["Быстроизнашиваемые детали",0.35,6000,24],["Крыльчатка вентилятора",1.56,15000,24],["Аккум. батарея",1.7,20000,24],["Капремонт двигателя",326.28,15000,24],["Ходовой мотор-редуктор",22.0,15000,24],["Редуктор хода",72.11,15000,24],["Ремонт форсунок",15.51,10000,24],["Стартер",9.41,10000,24],["Цилиндр стрелы",28.5,15000,24],["Цилиндр рукояти",18.0,15000,24],["Цилиндр ковша (левый)",11.5,15000,24],["Цилиндр ковша (правый)",11.5,15000,24],["Двигатель поворота",38.0,15000,24],["Поворотный редуктор",56.0,15000,24],["Центральный поворотный узел",26.5,25000,24],["Опора поворотного устройства",62.5,25000,24]]
CATALOGS=[
 # 0: PC2000-8
 [["Ремонт системы смазки",1.74,10000,24],["Ремонт системы кондиционир.",0.87,12000,24],["Чистка радиаторов",1.74,7000,24],["Звёздочка",15.77,17600,24],["Редуктор хода",471.3,17600,24],["Натяжное колесо",37.15,17600,24],["Гусеничная лента",0.02,17600,24],["Г. мотор хода",77.24,13200,24],["Поддерживающий каток",15.0,17600,24],["Опорный каток",54.61,17600,24],["Г. цилиндр стрелы",202.67,13200,24],["Г. цилиндр стрелы 2",172.32,13200,24],["Г. цилиндр рукояти",63.37,13200,24],["Г. цилиндр ковша",106.59,13200,24],["Г. распределитель",130.6,13200,24],["Г. распределитель 2",113.19,13200,24],["Г.насос управления",57.01,13200,24],["Г.насос",131.96,13200,24],["Г.мотор вентилятора",24.82,13200,24],["Г. мотор поворота",46.44,13200,24],["Поворотный круг",147.17,17600,24],["Редуктор поворота",177.83,13200,24],["ДВС",403.8,17600,24],["Механизм отбора мощности",231.28,17600,24],["Турбокомпрессор",9.57,8800,24],["Турбокомпрессор 2",8.26,8800,24],["Топл. насос",10.65,17600,24],["Топл. форсунка",16.28,8800,24],["Водяной насос",4.02,8800,24],["Генератор",5.77,15000,24],["Стартер",5.43,8800,24]],
 # 1: PC2000-11R
 [["Ходовые моторы",89.9,25000,24],["Редуктор хода",332.3,25000,24],["Гусеничная лента",184.0,20000,24],["Опорные ролики (комп)",119.41,20000,24],["Поддерживающие катки (комп)",31.38,20000,24],["Механизм натяжения гусениц",110.0,30000,24],["Натяжное колесо",80.0,20000,24],["Гидромотор поворота платформы",56.49,25000,24],["Редуктор поворота платформы",84.28,20000,24],["ДВС",320.0,25000,24],["Главные насосы, 2 шт.",317.54,25000,24],["Главный распределитель",162.12,25000,24],["Ремонт гидроцилиндров стрелы",18.33,12000,24],["Ремонт гидроцилиндра рукояти",11.2,12000,24],["Ремонт гидроцилиндра ковша",9.16,12000,24],["РВД рабочего оборудования",26.81,20000,24]],
 # 2: Zoomlion
 _Z,
 # 3: Sany (файла нет — каталог Zoomlion как заглушка, замените своим)
 [list(x) for x in _Z],
 # 4: Shantui (убраны сводные строки-дубли: «Замены ходовой», «Замена ДВС/мотора/редуктора/цилиндра/радиатора»)
...
for i in range(NEXC):
    ...
    # каталог узлов данной машины → текущие (ресурс<8000) и капитальные (ресурс≥8000)
    mitems=CATALOGS[i]
    cur_items=[x for x in mitems if x[2]<8000]
    cap_items=[x for x in mitems if x[2]>=8000]
    CURSEC=CORR+2; CURHDR=CURSEC+1; CUR0=CURHDR+1; CURN=CUR0+max(0,len(cur_items)-1)
    CAPSEC=CURN+2; CAPHDR=CAPSEC+1; CAP0=CAPHDR+1; CAPN=CAP0+max(0,len(cap_items)-1)
    NOTEROW=CAPN+2
    ...
    # ---- РАСЧЁТ РЕМОНТОВ (из списков по периодичности замены) ----
    section(ws,31,"РАСЧЁТ РЕМОНТОВ (из списков по периодичности замены узлов)","B","N")
    ...
    yline(33,"Текущие ремонты (из списка)","тыс/год",M,green=True,
          formula=lambda col:f"SUM({col}{CUR0}:{col}{CURN})*$E$36")
    ...
    section(ws,37,"КАТАЛОГ ЗАПЧАСТЕЙ ТО (заполняет подрядчик)","B","N")
    for k,h in enumerate(["Наименование","цена за ед","кол-во/ТО","№ ТО","стоимость,тыс/год"]):
        c=ws.cell(row=38,column=2+k,value=h); c.font=FHd; c.fill=Fh; c.border=BD; c.alignment=C
    ...
    cval=ws.cell(row=CORR,column=6,value=0); cval.fill=Fi; cval.border=BD; cval.alignment=C; cval.number_format=M2

    # ---- СПИСКИ РЕМОНТОВ ПО РЕСУРСУ ЗАМЕНЫ (периодичность) ----
    ws.column_dimensions["P"].width=10
    def repair_block(sec_row,hdr_row,items,r0,title_txt):
        section(ws,sec_row,title_txt,"B","N")
        for cc,h in ((2,"Узел / вид ремонта"),(3,"стоим.замены,тыс"),(4,"ресурс, м/ч")):
        ...
        ws.cell(row=hdr_row,column=16,value="нормо-час").font=FHd; ws.cell(row=hdr_row,column=16).fill=Fh; ws.cell(row=hdr_row,column=16).border=BD; ws.cell(row=hdr_row,column=16).alignment=C
        for k,(nm,cost,res,nh) in enumerate(items):
            r=r0+k
            ws.cell(row=r,column=2,value=nm).font=FN; ws.cell(row=r,column=2).border=BD; ws.cell(row=r,column=2).alignment=L
            ws.cell(row=r,column=3,value=cost); ws.cell(row=r,column=4,value=res)
            ...
            for p in range(1,H+1):
                col=PCOLS[p]
                f=ws.cell(row=r,column=4+p,value=f"=(INT($E$20*{p}/$D{r})-INT($E$20*({p}-1)/$D{r}))*$C{r}")
                f.fill=Fr; f.border=BD; f.alignment=C; f.number_format=M2; f.font=FRz
            nhc=ws.cell(row=r,column=16,value=nh); nhc.fill=Fi; nhc.border=BD; nhc.alignment=C; nhc.number_format=M
    repair_block(CURSEC,CURHDR,cur_items,CUR0,"ТЕКУЩИЕ РЕМОНТЫ — по ресурсу замены (затрата в годы кратные ресурсу)")
    repair_block(CAPSEC,CAPHDR,cap_items,CAP0,"КАПИТАЛЬНЫЕ РЕМОНТЫ (ППР) — по ресурсу до замены узла (данные поставщика)")

    tpl[i]={"sheet":TPL_NAMES[i],"ktg":11,"itogo":16,"fuel":17,"cons":18,"cur":"$D$5"}
    ws.merge_cells(f"B{NOTEROW}:P{NOTEROW}")
    nt=ws[f"B{NOTEROW}"]; nt.value=("Капремонты считаются ПО ПЕРИОДИЧНОСТИ, как у поставщиков: для каждого узла заданы "
     "стоимость замены и ресурс (м/ч); затрата возникает в те годы, когда наработка пересекает кратное ресурсу значение "
     "(число замен = ЦЕЛОЕ(наработка×год/ресурс) − ЦЕЛОЕ(наработка×(год−1)/ресурс)). Трудозатраты — нормо-часы × ставку. "
     "«Коэффициент поглощения» < 1 учитывает восстановление/поглощение узлов (у поставщика = 1).")
    nt.font=Font(size=9,italic=True,color="808080"); nt.alignment=L
print("Листы-шаблоны с моделью периодичности ремонтов:",NEXC)
